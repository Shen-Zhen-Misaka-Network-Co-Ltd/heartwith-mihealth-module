...
final class DebugSleepLog {
    ...
    private static final long KEEP_LOG_DAYS = 7L;
    ...
    static synchronized void init(Context context, String processName) {
        ...
        writeLine(processName, "debug-log init path=" + file.getAbsolutePath());
        if (!pathLogged) {
            pathLogged = true;
            Log.i(TAG, "debug log: " + file.getAbsolutePath());
        }
    }
    ...
    static synchronized void line(Context context, String processName, String message) {
        ...
        if (file == null || logDay == null || !logDay.equals(today)) {
            ...
            if (file != null) {
                pruneOldLogs(file.getParentFile());
                writeLine(processName, "debug-log day-open path=" + file.getAbsolutePath());
            }
        }
        if (file == null) {
            return;
    ...
    private static void writeLine(String processName, String message) {
        ...
        try {
            ...
            Log.w(TAG, "write sleep debug log failed: "
